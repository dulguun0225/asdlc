...
- **Status of this document:** assembled 2026-07-27 from the ADRs and
  [context.md](../context.md). It sequences decided things; where it decides something new —
...
- **One owner decision this plan cannot make:** which variant to *run*. Both are designed
  ([target-asdlc.md](target-asdlc.md) §9). They are not free to run simultaneously — every
...
([OQ-6](../open-questions.md), [OQ-7](../open-questions.md)), which favours the stack with
...
([ADR-0009](../adr/0009-code-host.md) consequences) — the wrong costs to pay
...
(SaaS permission withdrawn — [context.md](../context.md) currently permits it), or the owner
...
([OQ-10](../open-questions.md), [context.md](../context.md) "Not yet known").
...
1. **Name the platform owner and backup.** Neither may be an AI solution engineer on a
   ...
   [implementation.md](implementation.md) §L; every later step assumes it exists.
2. **State the deployment target.** Kubernetes → [ADR-0011](../adr/0011-progressive-rollout.md)
...
3. **Inventory the 18 engineers' operating systems; provision WSL2** for every Windows
   ...
   ([ADR-0007](../adr/0007-agent-runner-and-containment.md) part 3).
4. **Create the Claude Console organisation:** members with the restricted Claude Code
   ...
   ([ADR-0010](../adr/0010-runner-licensing-token-spend-only.md)); and a workspace rate
...
6. **Stand up observability:** OTel collector, the three record families, and the
   dashboards named in [implementation.md](implementation.md) §G. This precedes the pilot
...
- Managed settings distributed to every engineer machine; verified: sandbox refuses to run
  ...
  ([implementation.md](implementation.md) §E).
- The tier-function job, T3 proof checkers, requester check, and reassignment job
  implemented and tested against fixture repos (§B, §H, §D).
- Ruleset template, CODEOWNERS template, agent machine account (no write access), fork-PR
  ...
  implementation.md §M; until it verifies, the pipeline-level T1 gate — a per-push human
  authorisation against the current head commit,
  [implementation.md](implementation.md) §I — is mandatory.
...
[ADR-0005](../adr/0005-roles-gate-signers-and-the-reviewer-ring.md) rejected). A 3-ring with
...
- **Tokens and cost per unit of agent work** — the missing half of
  [OQ-7](../open-questions.md): per-session and per-merged-change spend, by model and by
  stage, from the Console workspace and OTel traces. This sets the per-tier session
  ceilings ([implementation.md](implementation.md) §F).
- **Per-tier gate baselines** — approval rate, change-request rate, defect attribution,
  ...
  ([OQ-6](../open-questions.md) baseline; drift needs time, but the baseline starts here).
...
[ADR-0009](../adr/0009-code-host.md) part 5 defines for the Gerrit case); the sandbox blocks
...
- Ring becomes the full 18-cycle at `k=1`; quarterly rotation through 1 → 5 → 7 → 11 → 13 →
  17 begins ([ADR-0005](../adr/0005-roles-gate-signers-and-the-reviewer-ring.md) part 4).
...
- The recorded review-competency list starts filling: team leaders who can sign plan gates
  ...
  ([ADR-0005](../adr/0005-roles-gate-signers-and-the-reviewer-ring.md) consequences).
...
- **T3 allowlist growth:** a path class moves down one tier at a time, as a reviewed T1
  act, on per-tier evidence ([ADR-0005](../adr/0005-roles-gate-signers-and-the-reviewer-ring.md)
...
- **T3 automatic deploy, per service:** all three exit conditions —
  ...
  [ADR-0011](../adr/0011-progressive-rollout.md)). Flip is a T1 change; services declared
...
- **Learned risk score:** only when incident history exists at volume; recorded as the far
  upgrade path ([ADR-0003](../adr/0003-graduated-gating-machine-derived-tier.md)), not a
  plan item.
...
- Phase 0 adds: Gerrit and Zuul installation and hardening; the access policy from
  [implementation.md](implementation.md) §J (no Push on `refs/heads/*`, no Forge Author);
...
- And the replacements, not only additions: phase 0 item 5 (the GitHub organisation) is
  ...
  [implementation.md](implementation.md) §J; the requester-check job (§H) is not built —
...
- Time-to-value is materially worse; that trade is the owner's to accept
  ([ADR-0009](../adr/0009-code-host.md) consequences).
...
in [OQ-7](../open-questions.md), and it is not our measurement).
