...
| **Is the design meant to be public?** `dulguun0225/asdlc` is **public today** — every ADR and [context.md](../reference/context.md) (team shape, roles, data boundary) is world-readable. **No record chooses this**; it is the current state, not a decision ([ADR-0026](../reference/decisions/0026-bundle-distribution.md) part 5) | **owner** — a disclosure call, not research | no — but **cheapest to change now**: nothing has been released, so no consumer depends on a public URL. Going private costs one `~/.specify/auth.json` per consumer and changes no URL |
