...
| **Where the bundle is published from.** All four `tools/spec-kit-bundle-nc/catalogs/*.json` point at `dulguun0225/spec-kit-bundle-nc` at tag `v0.2.0`, but the bundle now lives in `dulguun0225/asdlc`, which is **private** — release assets and raw catalog files both need public read. Dry-run 2026-07-28 with `GITHUB_REF_NAME=bundle-v0.2.0`: three release asserts pass, **all four catalog-URL asserts fail**. Three ways out, none taken yet: mirror the subtree to the public standalone repo and release there; make `asdlc` public and rewrite the catalogs; or leave it unpublished | **owner** — it turns on whether the ASDLC design and [context.md](../reference/context.md) may be public, which is a disclosure call, not a research question | **yes, before `bundle-v0.2.0` is tagged.** Nothing is stranded meanwhile: the standalone repo has **zero tags and zero releases**, so no consumer URL is live and rewriting the catalogs is still free |
