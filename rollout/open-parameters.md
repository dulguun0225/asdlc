...
| Signing-key generation, custody, backup and rotation runbook (self-hosted assembled; the integrated variant's equivalent waits on [OQ-22](../reference/open-questions.md#oq-22--provenance-on-the-integrated-self-hosted-variant)) | platform owner at bring-up — mechanism settled by [ADR-0018](../reference/decisions/0018-self-hosted-provenance.md) | **yes, self-hosted assembled** — losing the key makes every retained artifact undeployable |
| cosign attachment mode set **explicitly** to OCI referrers in both the signing job and the deploy verification | platform owner at bring-up | **yes, both self-hosted variants** — an inherited default makes the two disagree, which presents as "no attestation" and fails the deploy |
...
| Which credentials the agent needs, and their delivery as **environment variables** (a file credential cannot be masked) | platform owner at bring-up — constraint fixed by [ADR-0016](../reference/decisions/0016-tls-terminating-proxy-and-credential-masking.md) §4 | **yes, all variants** |
...
| Gerrit and Zuul licences (unrecorded) | verification ([assembled sheet](../variants/self-hosted.md) §3) | yes for the self-hosted assembled variant, which is defined by licence cost |
...
2. **What is the deployment target?** On Kubernetes, the progressive-rollout answer converges
   at zero licence cost. Off it, the self-hosted variants have **no verified mechanism** and
