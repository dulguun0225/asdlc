...
Two worked cases: `java-backend-api`, whose committed interface document is single conformance oracle a fuzzer generate against, and `money-java` `M-23`, which pin mutation testing scoped to money packages — **the scope is a named gap, stated on `M-23`'s own side: everything outside it sit green over vacuous tests.**
...
2. **The two wireable checks ship as description, not as code.** This skill carry no script a reader install, so a reader write or port both, and the earlier implementation the source material record — one script in that project's own build — is unpublished. A reference pair exist in the repository these skills distribute from (`https://github.com/dulguun0225/asdlc`, under `tools/skills-harness/scripts/`): `npm run gates`, each exiting non-zero and printing what it not decide, run by that repository's CI on every push touching the skills tree or the harness — a gate for that one skill set, a description for every other. **What building them produced is a finding about the rule, not about the files.** Measured 2026-08-02, re-runnable — **and it moved three times that same day, which is why the command matter more than any figure**: stated as *every subheading in the evidence file name a real section of the directive text*, the evidence-order rule hold over most of that set's evidence files and over none of the rest, which are organised by research pass — how the research happen. **Run `npm run check:evidence-order` there; it print both sides and name every exempt file.** Gate ship with the exempt files declared by name and reason, not with rule bent to fit.
