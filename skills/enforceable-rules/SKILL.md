...
2. **The two wireable checks ship as description, not as code.** **Narrowed 2026-08-02: both are now built and run over this skill set** — `npm run gates`, in the repository these skills distribute from (`https://github.com/dulguun0225/asdlc`, under `tools/skills-harness/scripts/`), each exiting non-zero and printing what it not decide. What stay a gap: this skill carry no script a reader install, so reader still write or port one; and the earlier implementation the source material record — one script in that project's own build — is still unpublished. **Narrowed 2026-08-05: the built pair now run automatically** — that repository's CI run both gates on every push touching the skills tree or the harness, so for that one skill set the pair is a gate rather than a command someone type; a reader's own set still have neither script nor CI until the reader wire them. **What building them produced is a finding about the rule, not about the files.** Measured 2026-08-02, re-runnable — **and it moved three times that same day, which is why the command matter more than any figure**: stated as *every subheading in the evidence file name a real section of the directive text*, the evidence-order rule hold over most of that set's evidence files and over none of the rest, which are organised by research pass — how the research happen. The two sides moved as skills gained evidence files and as one pass-organised file gained a directive-named section. **Run `npm run check:evidence-order` there; it print both sides and name every exempt file.** Gate ship with the 8 declared by name and reason, not with rule bent to fit.
