...
| *(stage-procedure delivery)* | Not a `tools/` program at all: the four stage procedures ship as Agent Skills from [asdlc/skills/](../asdlc/skills/README.md) via the **`skills` CLI** (`vercel-labs/skills`, MIT, external) | **Decided** — [ADR-0032](../reference/decisions/0032-stage-delivery-via-skills-cli.md), which rejected both a home-built renderer and `spec-kit-bundle/` for the role. Row kept so nobody re-invents it here |
