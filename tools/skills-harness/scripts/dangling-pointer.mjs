...
// Id prefixes that name numbering no installed skill dir carries. `P-n` was an
// earlier corpus's principle numbering and `B-n` a decision log's; both resolve
// for whoever wrote them and dangle for every reader. The ban is absolute, not
// contingent on whether a definition happens to exist.
const NEVER_SHIPS = /\b([PB]-\d+)\b/g;

// Filenames that name maintainer material rather than anything an installed
// skill dir carries. A skill citing one sends its reader to a file they do not
// have — and these three no longer exist in this repository either.
