...
<!--
  ...
  a condensed stock plan skeleton plus the three appended sections —
  Requirements Traceability, Decision Trace (showing all four row kinds:
  record citation, spec-fixed feature-local value, proposed new decision,
  recorded divergence), and the Approval section, always last. The
  decision-record paths are illustrative; in a real repo they point at
  files under docs/decisions/ or at the constitution's Repo principles. The approval was recorded by
  the human reviewer, which is what the speckit.nc.gate command verifies
  before implementation. It is a reference, not a template.
...
- Principles I and III: spec.md holds ten EARS requirements under stable
  FR-nnn IDs — verified.
- Principle II: this plan ends with the Approval section; implementation
  waits for it.
...
Approved — A. Reviewer (example), 2026-07-24
