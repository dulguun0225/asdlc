...
| **Cross-stack source** | **no — it has no seed file** | directives under stable ids, the evidence, and which stack packs have instantiated each. Lives in `rule-sources/`. [`money-grade`](rule-sources/money-grade.md) |
...
The corpus had nowhere to record a second source; this is it. A row here is a
topic, not a verdict: nothing below has had a research pass, and per B-8's
governance a source is written in the PR of the first repo that adopts it, never
ahead of it.

| Candidate source | Why a source and not a pack | Where its rules would land |
| ---------------- | --------------------------- | -------------------------- |
| **cache-discipline** | The engine pick is portable; the enforcement is not. A boundary rule on the cache client is an ArchUnit rule in Java and needs a different host in Go — one rule, a different tool per stack. | every stack pack whose repos run a cache |

Two distinctions this roster exists to settle before anyone researches
anything:

- **The engine pick is not a source rule.** Which cache a repo runs is one
  portable verdict with no check behind it, and the absent human reader does not
  change its stakes — it fails the premise-specificity test in
  [README.md](README.md) (Design principles). It is a line of seed text in each
  stack pack, or a plan decision at the first feature that needs one. "Not a new
  pack" below already routes a broker that way.
- **The discipline rules are.** A cache that is silently the source of truth, an
  entry with no TTL, a failure that falls back instead of failing loud — each
  turns invisible-forever once no human reads the code, and each needs a
  different named check per stack. That is this section's shape exactly.

The same split is expected for the other shared infrastructure concerns —
message broker, object storage, search index, feature flags. `money-grade` is
the first source, not a money-only special case.
...
[`money-grade`](rule-sources/money-grade.md), so it is live, and it would still
be live on a twelve-month sweep that found no adopting repo. The `review-by`
freshness rule and the lapse rule apply to a source unchanged.
