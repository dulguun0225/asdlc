...
<!--
  ...
  extension hooks, feature-directory creation, template resolution, the quality
  checklist, and the completion report all run unchanged. The sections here add
  EARS authoring and validation to it.
-->
...
You shall not write `**Status**: Approved` anywhere. A new or edited spec is a
Draft; a human reviewer records the approval (see the spec template's Status
comment). If the user asks you to mark the spec approved, decline and explain
that the approval line records who reviewed it, so the reviewer writes it.
