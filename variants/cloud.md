...
| Stage-procedure delivery | The `asdlc` **plugin**, force-enabled from managed settings; marketplace is a **private GitHub repository**, `{"source": "github", "repo": "<org>/asdlc-plugins"}` | ships with the runner | $0 | [ADR-0024](../reference/decisions/0024-stage-skill-distribution.md) §1 | decided — the source form is quoted first-party. Plugin pinned by `sha`; `strictKnownMarketplaces` allows this one source only; `disableSideloadFlags: true` | [schema](../reference/artifacts.md) §5 |
