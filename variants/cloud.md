...
| T1 path ownership | **CODEOWNERS** on the map's rule-1/rule-2 paths — the owning team's engineer and team leader | included | $0 | [ADR-0009](../reference/decisions/0009-code-host.md) §1 | decided | §5 below |
...
| T1 pre-run CI gate | mechanism undecided — fork-PR approval is documented for public repos only | included | $0 | [ADR-0009](../reference/decisions/0009-code-host.md) §1 | **open parameter** — operator at bring-up ([open parameters](../rollout/open-parameters.md)); pipeline-level gate covers the interim | §5 below |
...
- **Audit watch:** alert the operator on `protected_branch.policy_override` and on
