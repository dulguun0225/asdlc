...
### Code host, gates and registry — one forge instead of three systems
...
model to train; the ring/reassignment job speaks one model through two APIs).
...
| Audit record | **none native** — external logging of webhook-visible events | — | $0 | [ADR-0039](../reference/decisions/0039-self-hosted-forks-on-the-assembly-axis.md) | decided — **the variant's accepted loss 1.** Upgrade trigger: [forgejo#6982](https://codeberg.org/forgejo/forgejo/issues/6982) ships | §4 below |
...
### Build and provenance — the named gap
...
|---|---|---|
| Provenance signer, key custody, predicate, verification | cosign remains the signer and SLSA v1 the predicate ([ADR-0018](../reference/decisions/0018-self-hosted-provenance.md) §1–2 carry over as mechanism) — but **where the key lives and what protects it from a proposed workflow change is undecided** | **GAP — [OQ-22](../reference/open-questions.md#oq-22--provenance-on-the-integrated-self-hosted-variant).** Must close before this variant's first production deploy, not before its pilot |
| Transparency log | none, as in the assembled variant | decided ([ADR-0018](../reference/decisions/0018-self-hosted-provenance.md) §6) |
| Artifact addressing | **digest, never tag** | decided ([ADR-0017](../reference/decisions/0017-artifact-registry.md) §4) |
